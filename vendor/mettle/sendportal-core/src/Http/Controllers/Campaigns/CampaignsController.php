...
class CampaignsController extends Controller
{
    ...
    public function index(): ViewContract
    {
        ...
        $campaigns = $this->campaigns->paginate($workspace->id, 'created_atDesc',[],1000, ['status']);
     ...
     * @throws Exception
     */
    public function store_special_camp(CampaignStoreRequest $request): RedirectResponse
    {
        $campaign = $this->campaigns->store(auth()->user()->currentWorkspace()->id, $this->handleCheckboxes($request->validated()));
    ...
    public function edit(int $id): ViewContract
    {
        ...
        $emailServices = $this->emailServices->all(auth()->user()->currentWorkspace()->id);
        $templates = [null => '- None -'] + $this->templates->pluck(auth()->user()->currentWorkspace()->id);
        $segments = $this->segments->all(auth()->user()->currentWorkspace()->id)->pluck('name','id');

        return view('sendportal::campaigns.edit', compact('campaign', 'emailServices', 'templates','segments'));
    }
  ...
  /**
     ...
     * @throws Exception
     */
    public function preview_special(int $id)
    {
     	$campaign = $this->campaigns->find(auth()->user()->currentWorkspace()->id, $id);
        $subscriberCount = $this->subscribers->countActive(auth()->user()->currentWorkspace()->id);
        ...
        return view('sendportal::campaigns.preview_special', compact('campaign', 'segments', 'subscriberCount'));
    }
