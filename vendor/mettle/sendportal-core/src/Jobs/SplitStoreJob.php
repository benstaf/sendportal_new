...
// create n new segments and get their IDs
$new_segments = [];
for ($i = 1; $i <= $num_segments; $i++) {
    $name_split_i = $name_split . " #$i";
    $data_i = ['_token' => $token, 'name' => $name_split_i];
    $segment_i = $this->segmentRepository->store($this->workspace_id, $data_i);
...
//$this->segmentRepository->attach($attachData);


// perform bulk insertion
$this->segmentRepository->bulkAttachSubscribers($attachData);
